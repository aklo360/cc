 ...
 */
export function isUrlExpiringSoon(thresholdMs = 30 * 60 * 1000) {
    if (!cachedUrl)
        return true;
...
async function fetchFreshUrl() {
    try {
        ...
        const { stdout, stderr } = await execAsync(`yt-dlp -f 91 -g "${LOFI_STREAM_URL}"`, { timeout: URL_FETCH_TIMEOUT });
